...
void readfile_fun(const Char_t* run_name,const Int_t numberOfFiles,const Char_t * outputname)
{
  ...
  for(Int_t ientry=0;ientry<nentries;ientry++)
    ...
    for(Int_t j=0;j<theDDASevent->GetNEvents();j++){
      ...
      ddaschannel * thechannel=theDDASevent->GetData()[j];
...
void DDAS2Event(BetaDecay *bdecay){
  ...
  //for(Int_t jevent=0;jevent<sizeOfEvent;jevent++){
    ...
    Int_t theCrate=(*channellist_it)->GetCrateID();
    Int_t theSlot=(*channellist_it)->GetSlotID();
    Int_t theChannel=(*channellist_it)->GetChannelID();
    UInt_t theEnergy=(*channellist_it)->GetEnergy();
    UInt_t theTimeLow=(*channellist_it)->GetTimeLow();
    UInt_t theTimeHigh=(*channellist_it)->GetTimeHigh();
    UInt_t theTimeCFD=(*channellist_it)->GetCFDTime();
    Double_t theTime=(*channellist_it)->GetTime();
